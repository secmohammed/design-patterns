...
func (f *FormattedText) Captialize(start, end int) {
    for i := start; i <= end; i++ {
        f.capitalize[i] = true
    }
}

// Flyweight pattern is represesnted here, due to we can optiimzie our memory throughout this solution
// (space-optimization)
type TextRange struct {
    Start, End               int
    Captialize, Bold, Italic bool
}
...
func main() {
    ...
    // this design pattern introduces a trick we must do to
    // optimize memory, even though we might declare temp variables/objects
    // but they allow us to save memory.
    bft := NewBetterFormattedText(text)
    bft.Range(16, 19).Captialize = true
